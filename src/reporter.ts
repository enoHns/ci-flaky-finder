...
import { FlakyReport, FlakyTest } from './types'
...
function buildComment(report: FlakyReport): string {
  const lines: string[] = [COMMENT_TAG]
  ...
  if (report.flaky.length === 0) {
    ...
    const shortHint: Record<string, string> = {
...
function formatJobDetail(t: FlakyTest): string {
  const lines = [
    `### \`${t.jobName}\``,
    `- **Step**: \`${t.stepName}\``,
